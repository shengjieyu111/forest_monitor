...
def _set_mr_state(**updates):
    with _MR_JOB_LOCK:
        _MR_JOB_STATE.update(updates)


def _append_mr_step(step):
    with _MR_JOB_LOCK:
        _MR_JOB_STATE["steps"].append(step)


def _update_mr_step(step_name, **updates):
    with _MR_JOB_LOCK:
        for step in _MR_JOB_STATE["steps"]:
            if step.get("step") == step_name:
                step.update(updates)
                break


def _run_visitor_mr_pipeline():
    jobs = [
    ...
    try:
        upload_step = {
            ...
            "name": "上传并检查 HDFS 日期分区输入目录",
            "success": None,
            "running": True,
        }
        _append_mr_step(upload_step)
        try:
            _update_mr_step(
                "upload_visitor_csv",
            ...
            _set_mr_state(running=False, success=False, message="HDFS 输入准备失败")
            return

        raw_import_step = {
            "step": "import_visitor_records",
            "name": "同步游客原始数据到 MySQL",
            "success": False,
        }
        try:
            ...
            if existing_rows:
                ...
                raw_result = import_visitor_records_from_csv(
                    csv_path=csv_path,
            ...
            _append_mr_step(raw_import_step)
            _set_mr_state(running=False, success=False, message="游客原始数据导入失败")
            return
        _append_mr_step(raw_import_step)

        for job_key, job_name, run_remote, import_result in jobs:
            remote_step_name = f"run_{job_key}_mapreduce"
            remote_step = {
                "step": remote_step_name,
                "name": f"运行{job_name} MapReduce 并下载结果",
                "success": None,
                "running": True,
            }
            _append_mr_step(remote_step)
            try:
                _update_mr_step(
                    remote_step_name,
                    success=True,
            ...
            import_step_name = f"import_{job_key}_result"
            import_step = {
                "step": import_step_name,
                "name": f"导入{job_name}结果到 MySQL",
                "success": None,
                "running": True,
            }
            _append_mr_step(import_step)
            try:
        ...
        _set_mr_state(running=False, success=False, message=f"后台任务异常：{exc}")


def visitor_mr_status_api(request):
    with _MR_JOB_LOCK:
        data = dict(_MR_JOB_STATE)
        data["steps"] = list(_MR_JOB_STATE["steps"])
    return JsonResponse(
        {"code": 0, "success": data.get("success") is not False, "data": data},
        json_dumps_params={"ensure_ascii": False},
    )
...
def run_all_mr_and_import_api(request):
    with _MR_JOB_LOCK:
        if _MR_JOB_STATE["running"]:
            return JsonResponse(
                {
                    "code": 0,
                    "success": True,
                    "message": "MapReduce 正在运行",
                    "data": dict(_MR_JOB_STATE),
                },
                json_dumps_params={"ensure_ascii": False},
            )
        _MR_JOB_STATE.update(
            {
                "running": True,
                "success": None,
                "message": "MapReduce 正在运行",
    ...
    thread = threading.Thread(target=_run_visitor_mr_pipeline, daemon=True)
    thread.start()
    return JsonResponse(
        {
            "code": 0,
            "success": True,
            "message": "MapReduce 已开始后台运行",
            "data": {"running": True, "success": None, "steps": []},
        },
        json_dumps_params={"ensure_ascii": False},
    )
