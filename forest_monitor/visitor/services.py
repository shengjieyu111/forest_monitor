...
REMOTE_DATASET_DIR = getattr(
    settings,
    "HADOOP_REMOTE_DATASET_DIR",
    "/root/forest_monitor/datasets",
)
# HDFS 数据按 date=YYYY-MM-DD 分区存储，MapReduce 从根目录递归读取。
HDFS_INPUT_PATH = getattr(settings, "VISITOR_HDFS_INPUT_PATH", "/waether/visitor/input")
HDFS_OUTPUT_ROOT = getattr(settings, "VISITOR_HDFS_OUTPUT_ROOT", "/waether/visitor/output")
HDFS_MR_INPUT_PATH = getattr(settings, "VISITOR_HDFS_MR_INPUT_PATH", "/waether/visitor/mr_input")
...
def _detect_visitor_csv_date(csv_path):
    date_from_name = csv_path.stem.removeprefix("visitor_today_")
    if date_from_name != csv_path.stem:
        try:
            datetime.strptime(date_from_name, "%Y-%m-%d")
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as csv_file:
        ...
        if first_row and first_row.get("visit_time"):
            return first_row["visit_time"][:10]

    return timezone.localdate().isoformat()


def _select_visitor_csv_for_hdfs():
    dated_files = sorted(LOCAL_DATASET_DIR.glob("visitor_today_????-??-??.csv"))
    candidates = [
        LOCAL_DATASET_DIR / "visitor_records.csv",
        LOCAL_DATASET_DIR / "visitor_today.csv",
        *dated_files,
    ]
    for csv_path in candidates:
        if csv_path.exists():
            return csv_path, _detect_visitor_csv_date(csv_path)

    raise FileNotFoundError("未找到游客 MapReduce 输入 CSV")


def _split_visitor_csv_by_date(csv_path, output_dir):
    ...
    try:
        with Path(csv_path).open("r", encoding="utf-8-sig", newline="") as csv_file:
            ...
            for row in reader:
                total_rows += 1
                visit_time = (row.get("visit_time") or "").strip()
                try:
                ...
                if date_str not in writers:
                    partition_path = output_dir / f"visitor_records_{date_str}.csv"
                    file_obj = partition_path.open("w", encoding="utf-8", newline="")
                    writer = csv.DictWriter(file_obj, fieldnames=CSV_FIELDS)
                    writer.writeheader()
                    files[date_str] = file_obj
                    writers[date_str] = writer

                writers[date_str].writerow(
    ...
    partition_files = {
        date_str: output_dir / f"visitor_records_{date_str}.csv"
        for date_str in sorted(writers)
    }
    return partition_files, total_rows, skipped_rows


def _hdfs_visitor_input_exists():
    command = " && ".join(
        [
            f"hdfs dfs -test -d {shlex.quote(HDFS_INPUT_PATH)}",
            (
                f"hdfs dfs -find {shlex.quote(HDFS_INPUT_PATH)} "
                "-name visitor_records.csv | head -1"
            ),
        ]
    )
    try:
...
def _prepare_visitor_mr_input(csv_path):
    remote_csv_path = f"{REMOTE_DATASET_DIR}/{csv_path.name}"
    hdfs_csv_path = f"{HDFS_MR_INPUT_PATH}/visitor_records.csv"
    command = " && ".join(
        [
            f"mkdir -p {shlex.quote(REMOTE_DATASET_DIR)}",
            f"hdfs dfs -rm -r -f {shlex.quote(HDFS_MR_INPUT_PATH)}",
            f"hdfs dfs -mkdir -p {shlex.quote(HDFS_MR_INPUT_PATH)}",
        ]
    )
    run_ssh_command(f"bash -lc {shlex.quote(command)}")
    _upload_local_file(csv_path, remote_csv_path)
    run_ssh_command(
        f"bash -lc {shlex.quote(
            f'hdfs dfs -put -f {shlex.quote(remote_csv_path)} {shlex.quote(hdfs_csv_path)}'
        )}"
    )
...
def upload_visitor_csv_to_hdfs_remote():
    csv_path, _date_str = _select_visitor_csv_for_hdfs()
    mr_input = _prepare_visitor_mr_input(csv_path)
    exists, sample_file = _hdfs_visitor_input_exists()
    if exists:
        return {
            ...
            "skipped": True,
            "mode": "date_partition_existing",
        }

    remote_partition_dir = f"{REMOTE_DATASET_DIR}/visitor_partitions_{csv_path.stem}"

    with tempfile.TemporaryDirectory(
        prefix="visitor_partitions_", dir=str(LOCAL_DATASET_DIR)
    ) as temp_dir:
        partition_files, total_rows, skipped_rows = _split_visitor_csv_by_date(
            csv_path, temp_dir
        )
        if not partition_files:
            raise ValueError("游客 CSV 中没有可上传的有效 visit_time 日期")

        setup_command = " && ".join(
            [
                f"rm -rf {shlex.quote(remote_partition_dir)}",
                f"mkdir -p {shlex.quote(remote_partition_dir)}",
                f"hdfs dfs -rm -r -f {shlex.quote(HDFS_INPUT_PATH)}",
                f"hdfs dfs -mkdir -p {shlex.quote(HDFS_INPUT_PATH)}",
            ]
        )
        ...
        try:
            with client.open_sftp() as sftp:
                for local_file in partition_files.values():
                    remote_path = f"{remote_partition_dir}/{local_file.name}"
        ...
        command = "\n".join(
            [
                "set -e",
                f"for f in {shlex.quote(remote_partition_dir)}/visitor_records_*.csv; do",
                '  base="$(basename "$f")"',
                '  date="${base#visitor_records_}"',
                '  date="${date%.csv}"',
                f"  partition={shlex.quote(HDFS_INPUT_PATH)}/date=\"$date\"",
                '  hdfs dfs -mkdir -p "$partition"',
                '  hdfs dfs -put -f "$f" "$partition/visitor_records.csv"',
                "done",
                f"hdfs dfs -ls -h {shlex.quote(HDFS_INPUT_PATH)}",
            ]
        )
    ...
    return {
        ...
        "date_start": min(partition_files),
        "date_end": max(partition_files),
        "total_rows": total_rows,
        "skipped_rows": skipped_rows,
        "hdfs_status": command_result.stdout,
        "mode": "date_partition_split",
    }
...
def _run_mapreduce_remote(
    ...
    run_ssh_command(f"bash -lc {shlex.quote(prepare_command)}")
    _upload_local_file(local_jar_path, jar_path)

    command = " && ".join(
        [
            f"hdfs dfs -rm -r -f {shlex.quote(hdfs_output_path)}",
            f"hadoop jar {shlex.quote(jar_path)} {shlex.quote(main_class)} {shlex.quote(HDFS_MR_INPUT_PATH)} {shlex.quote(hdfs_output_path)}",
